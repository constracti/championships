...
function ScheduleMatchesDefault(matches,rounds,slots,sports){
	if (matches.length === 0){
		return true;
	}
	else{		
					...
					if (slots[s].available === true && slots[s].round.rank===rounds[r].rank){// *in this round
						...
						for (let sp = 0; sp < sports.length; sp++){
							for (let j = 0; j < rounds[r][sports[sp].name].length; j++){
								if (rounds[r][sports[sp].name][j].match !== null){
									...
									if (rounds[r][sports[sp].name][j].match.team_home.name === team1 || rounds[r][sports[sp].name][j].match.team_away.name === team1 || rounds[r][sports[sp].name][j].match.team_home.name === team2 || rounds[r][sports[sp].name][j].match.team_away.name === team2){
						...
						for (let sl = 0; sl < slots.length; sl++){
							if (slots[sl].match !== null){
								if ((slots[sl].match.team_home.name === team1 || slots[sl].match.team_away.name === team1) && (slots[sl].match.team_home.name === team2 || slots[sl].match.team_away.name === team2) && (slots[sl].match.sport.name === slots[s].sport.name)){
									scheduled = true;
									break;
								}
							}
